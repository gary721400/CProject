...
//	Find_PathFile( FILE_MODE_LOCAL, MP3_PATH );
	memset(current_mp3_path,0,32);
	if(b_sd1_ok){
		//strcpy(current_mp3_path,SD1_MP3_FILE);
		strcpy(current_mp3_path,mp3DirName);
//		lFileMax_SD1_WAV = file_get_ext_count(current_mp3_path, WAV_EXT);
		lFileMax_SD1_MP3 = file_get_ext_count(current_mp3_path, MP3_EXT);
		}
	if(b_sd2_ok){
		strcpy(current_mp3_path,SD2_MP3_DIR);
...
*/			  
	...
	current_playmp3_number = 0;
	g_b_secondgo_mp3 = FALSE;
	  if(g_b_secondgo_mp3){
...
//        		LED_param = 600;
    	...
    	case (STATE_KEY_PRESSED|KEY_MODE): // mode
    		//if(flag){
			//	AudioMP3Stop();
			//	flag_repeat = TRUE;
			//	//AudioMP3FileCloseAfter();
...
Bool find_wav(unsigned int num,Bool flag)
{
	...
	if(flag){
		strcpy(current_wav_path,SD2_MP3_DIR);
		}
	else{
		strcpy(current_wav_path,mp3DirName);
		}
...
//		SDKLoadCode(FILE_EXTERN_CODE);
		...
		(void)find_path_u((U16 *)&tisiFileName,(U8 *)mp3DirName);
		//*/
#else		
		strcpy(mp3DirName,(char *)SD1_MP3_DIR);
#endif		

#ifdef TX_DEBUG
		SendString(( char *)"\r\nmp3DirName=");
		SendString(mp3DirName);
#endif
}
