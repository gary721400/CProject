...
void	local_to_unicode_kb(U8 * local_str,U16 * unicode_str,U16 buffer_size)
{
	while(*local_str)
	{
...
int	str_comp_u_kb(U16  * sl,U16  * sr,U8 use_case)
{
	if(use_case)
	{
		while(*sl&&*sl==*sr)
...
Bool	kb_fat_buffer_load(KBFatBuffer * b,U8 disk_id,U32 sector_addr)
{
	...
	if(disk_id!=b->disk_id||sector_addr!=b->sector_addr)
	{
		...
		if(sd1_read(sector_addr,(__X char *)b->buffer)!=0)
			return	FALSE;
		b->disk_id = disk_id;
...
Bool	kb_fat_buffer_flush(KBFatBuffer * b)
{
	if(b->is_modified)
	{
		...
		if(sd1_write(b->sector_addr,(__X char *)b->buffer)!=0)
			return	FALSE;
		b->is_modified = FALSE;
	}
	return	TRUE;
...
int	kb_fat_check_cluster_subdir(FSINFO * fs_info,U32 cluster)
{
...
Bool	kb_fat_dir_entry_next(KBFatDir * dir)
{
	...
	if(slot_index%Fat_Entries_Per_Sec==0)
	{
		...
		{
			if(kb_fat_check_cluster_subdir(dir->fs_info,dir->cur_cluster)==Fat_Valid_Cluster)
			{
				cluster = GetFATEntry(dir->data_buf.disk_id,dir->cur_cluster);
				check = kb_fat_check_cluster_subdir(dir->fs_info,cluster);
...
Bool	kb_fat_dir_sub(KBFatDir * dir)
{
		...
		&&(ptr[KB_Fat_Dir_Attr_Offset]&KB_Fat_Dir_Attr_Directory))
	{
		...
		if(kb_fat_check_cluster_subdir(fs_info,cluster)!=Fat_Valid_Cluster)
			return	FALSE;
...
Bool	kb_fat_path(KBFatDir * dir,U16 * path,U16 * long_name,U8 * short_name)
{
	...
	while(*left)
	{
		...
		*right = 0;
		...
		{
			local_to_unicode_kb(short_name,sn,sizeof(sn));
			if(str_comp_u_kb((U16 *)long_name,(U16 *)left,FALSE)==0||
				str_comp_u_kb((U16 *)sn,(U16 *)left,FALSE)==0)
			{
