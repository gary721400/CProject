...
__X   U32   u32_oid_read_timeout = 0;

Bool  g_oid = FALSE;

//__X   U32   Current_Index;

__X   U32   u32_oid_read_time = 0;
...
U16 read_oid_pen( void )
{
   ...
   oid_sdio_input();
   if( from_pen_data == OID1Cmd_PowerDown )
      exit_flag = TRUE;
   ...
   if( !read_sdio())
   {
      //if( !bPenOn )
      {
         ...
         if( !( data_from_pen & 0x8000 ))
         {
            ...
            if(( data_from_pen == OID1_NULL_INDEX )
