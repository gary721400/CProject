...
//
#define SPLASH_TIME           (TIMER_MS(50))  //! Delay of the splash screens
#define PLAYER_TIME           (TIMER_MS(200)) //! Period of the screen refreshing in mmi application Player
#define STORAGE_ARROW_TIMER   (TIMER_MS(300)) //! Refresh delay of the USB R/W animation in mmi application Storage 
...
#define EQUALIZER_DELAY       (TIMER_S(4))    //! Display duration of the volume window in mmi application Equalizer 
#define PROPERTIES_DELAY      (TIMER_S(4))    //! Display duration of the properties window in mmi application Properties
#define INFO_DELAY            (TIMER_S(4))    //! Delay to switch off the Info mmi
#define MAINMENU_TIME         (TIMER_MS(500)) //! Delay between main menu pictures
#define STORAGE_IDLE_TIMER    (TIMER_MS(700)) //! Delay to detect U-disk idle mode
#define SETTING_MENU_TIME     (TIMER_S(5))    //! Time given between each keypressed. If elapsed, go back to the main menu
