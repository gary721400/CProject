...
code CDB_FUN VendorCmdFun[ MAX_VENDOR_COMMANDS ]=
{
   ...
   {  OP_GET_DEVICE_INFO,  /*OP_Vendor_Null*/OP_Vendor_GetDeviceInfo },
   {  OP_null_03,          OP_Vendor_Null          },
   {  OP_null_04,          OP_Vendor_Null          },
   {  OP_null_05,          OP_Vendor_Null          },
   {  OP_null_06,          OP_Vendor_Null          },
   {  OP_NOR_SectorErase,  OP_Vendor_Null/*OP_Vendor_SectorERASE*/   },
   {  OP_NOR_WRITE512,     OP_Vendor_Null/*OP_Vendor_NorWrite512*/   },
   {  OP_NOR_READ512,      OP_Vendor_Null/*OP_Vendor_NorRead512*/    },
   {  OP_NOR_ID,           OP_Vendor_Null/*OP_Vendor_ReadNorID*/     },
   {  OP_null_0c,          OP_Vendor_Null          },
   {  OP_null_0d,          OP_Vendor_Null          },
   {  OP_null_0e,          OP_Vendor_Null          },
   {  OP_null_0f,          OP_Vendor_Null          },
   {  OP_SRAM_READ,        /*OP_Vendor_Null*/OP_Vendor_SramAccess    },
   {  OP_SRAM_WRITE,       /*OP_Vendor_Null*/OP_Vendor_SramAccess    },
...
void OP_Vendor_ReadNorID( void ) // none call in the SW .. ng
{
   ...
   XBYTE[ 0xB000 ] = ( unsigned char )( PMC_ID >> 8 );
   XBYTE[ 0xB001 ] = ( unsigned char )( PMC_ID & 0x00FF );
...
#define bufSel    cdb[ 5 ]
      ...
      if( cdb[ 1 ] == OP_SRAM_READ )
      {
         ...
         memcpy(( UCHAR xdata * )BUFFER0_ADDR, ( UCHAR xdata * )address.INTEGER, length );
      ...
      else
      {
         ...
         memcpy(( UCHAR xdata * )address.INTEGER, ( UCHAR xdata * )BUFFER0_ADDR, length );
      }
...
#undef bufSel
   ...
   else
   {
      ...
      else
      {
         ...
         memcpy(( UCHAR data * )address.INTEGER, ( UCHAR xdata * )BUFFER0_ADDR, length );
      }
   }
#undef address
}
