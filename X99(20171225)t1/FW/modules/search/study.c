...
#endif
ImageDeclareType( BROWER_PAGE_INIT_CODE );
ImageDeclareType( BROWER_PAGE_CODE );
ImageDeclareType( DISPLAY_PAGENUM_CODE );
ImageDeclareType( HAVE_KEYPRESS_CODE );
ImageDeclareType( PRINTUNITENEWS_CODE );
...
_MEM_TYPE_SLOW_  U16              findword_num;
...
extern _MEM_TYPE_SLOW_      U8                time_buf[ 10 ];
...
void brower_page                    ( void );
Bool have_keypress(void);
void ready_goto_page(Bool flag);
void printUniteNews(void);
//***********************************************
...
_MEM_TYPE_SLOW_   Unite_News           Cur_Unite_News;
//_MEM_TYPE_SLOW_   Jian_News              Cur_Jian_News;
//_MEM_TYPE_SLOW_   Run_Unite_News     Cur_Run_News;
_MEM_TYPE_SLOW_   All_Unite_News       All_unite_news;
_MEM_TYPE_SLOW_   U16                  Cur_Cap_Num;
...
//_MEM_TYPE_MEDSLOW_   U16               ap4current_displayfile_num = 0;
...
extern _MEM_TYPE_MEDSLOW_   U16               dummy_index;
//***********************************************
#if 1
...
void search_evt( void )
{
    ...
    switch( State_Search ) {
        ...
        case Book_Search:
            ...
            break;
///*
        case Browser_Page_Init:
            SDKLoadCode( BROWER_PAGE_INIT_CODE );
            brower_page_init();
            break;

        case Browser_Page:
            SDKLoadCode( BROWER_PAGE_CODE );
            brower_page();
            break;
//*/
        case Page_Search:
            SDKLoadCode(PRINTUNITENEWS_CODE);
            printUniteNews();
        ...
        case Normal_Search:
            SDKLoadCode( HAVE_KEYPRESS_CODE );
            //if(have_keypress())return;
            (void)have_keypress();
...
#pragma CODE = HAVE_KEYPRESS_CODE
Bool have_keypress(void)
{
    U32 temp_add,temp_addr;
    ...
    if(b_HaveKeyPress) {
        ...
        player_ap3_stop();
        init_array_v();
        b_HaveKeyPress = FALSE;
        //temp_add = Cur_Jian_News.Point_Default + (U32)((Cur_Run_News.Index_ID - Cur_Jian_News.Min_Code)<<2);
        //temp_add = Cur_Unite_News.Point_UniteNews + (U32)((Cur_Run_News.Index_ID - Cur_Unite_News.Min_PageId)<<2);

        temp_add = Cur_Unite_News.Point_UniteNews + (U32)((Cur_Cap_Num)<<2);
        //SendString((U8 *)"\r\nadd=");
        //send_hex_u32(temp_add);
        read_data_from_nf(temp_add, (U8 _MEM_TYPE_SLOW_ *)&temp_addr, 4);

        (void)act_do(temp_addr, 0);
        state_search_to_play();
        return TRUE;
    }
    return FALSE;
...
//  U8 _MEM_TYPE_SLOW_ text_buf[ TEXT_BUF_LEN ];
    ...
    /*
       if(Current_Act.Type == Type_Play){
        leng_ch = News_Book.Len_Cap;
        ch_begin_address = Addr_Cap_Book;
        }
       else{
        */
...
Bool record_judge( void )
{
    ...
    if(( Current_Index >= Ap4_Record_Range.Play_Index_Min )&&( Current_Index <= Ap4_Record_Range.Play_Index_Max )) {
        ...
        //rp_flag_bits.value &= ~0x0F;
        rp_flag_bits.value = 0;
        rp_oid_index = Current_Index - Ap4_Record_Range.Play_Index_Min + Ap4_Record_Range.Record_Index_Min;
...
void search_book_deal( void )
{
    ...
    if(News_Book.Point_UniteTable != 0) {
        read_data_from_nf( News_Book.Point_UniteTable, ( U8 _MEM_TYPE_SLOW_ *)&All_unite_news.Unite_Num, 6 );
    } else {
        All_unite_news.Cap_Point = 0;
        All_unite_news.Unite_Num = 0;
    }
    ...
    send_hex_u16(All_Control.Version);
    SendString((U8 *)":");
    send_hex_u32(News_Book.Point_UniteTable);
    SendString((U8 *)":");
    send_hex_u16(All_unite_news.Unite_Num);
    //*/
...
//  State_Search = Browser_Page_Init;
//  return;
    //ap4current_displaypage_num = 0;
    gStudyUnite = 0;
    if(Point_Read == Point_Write) {
        ...
        ap4_play_state = AP4_PLAY_BROWSER_PAGE;
        mmi_ap3_kbd_disable();
...
#endif
    /*
    SendString((U8 *)"\r\nap4current_displaypage_num:num:cap:mask=");
    send_hex_u16(gStudyUnite);
    SendString((U8 *)":");
    //send_hex_u32(voice_addr);
    send_hex_u16(Cur_Unite_News.Cur_Unite_Num);
    //SendString((U8 *)":");
    send_hex_u16(Cur_Unite_News.Cap_Num);
    //SendString((U8 *)":");
    //send_hex_u16(Cur_Unite_News.Min_PageId);
    //SendString((U8 *)":");
    //send_hex_u16(Cur_Unite_News.Max_PageId);
    //SendString((U8 *)":");
    send_hex_u32(Cur_Unite_News.Point_UniteNews);
    */
    //SendString((U8 *)":");
    //send_hex_u16(ap4current_mark);

    //ap4CurrentMarkBak = ap4current_mark;
    // bStudyUniteMask = true;
    SendString((U8 *)"\r\nPage");
    reread_enable_flag = FALSE;
    g_b_first_n_p_key_flag = TRUE;
    ap3_rand_init();
    if( bGameClicked ) {
        bGameClicked = FALSE;
        /*
    ...
    page_search_deal();
    dummy_index = News_Go.page_info.Min_Index_ThePage;
}
    ...
    if( Point_Write != Point_Read ) {
        ...
        if( player_ap3_start( Array_V[ Point_Read ].Addr_Speech, Array_V[ Point_Read ].Flag_Speech )) {
            ...
            if( !bInCal
                ...
                mmi_ap3_display_icon(FALSE);
                if(ap4_play_state == AP4_PLAY_BROWSER_PAGE) {
...
//     lcd_goto_xy( FILEBROWSER_PosFile_StartX, FILEBROWSER_PosFile_StartY );
    lcd_clear_part_of_screen( 0, 24, LCD_WIDTH, (LCD_HEIGHT-24));
    //temp_file = (display_num_mark-1)*DICTEXT_LINE_PAGE;
    if(0 != (display_num_mark%DICTEXT_LINE_PAGE)) {
        display_num_mark -= 1;
    }
    //temp_file = display_num_mark + 1;
    temp_file = display_num_mark;
...
///*
    ...
    SDKLoadCode( LCD_SSD1303_TEMP_CODE02 );
    lcd_clear_part_of_screen( 0, 24, 10, (LCD_HEIGHT-24));
    lcd_draw_picture(5,32+20*(gStudyAp4%DICTEXT_LINE_PAGE), IDP_PLAYER_PLAY);
   // */
}
...
void brower_file(void)
{
    ...
    switch( browserfile_cmd_id ) {
        case BROWSER_Cmd_CurFile:
            AP4_filename_display(gStudyAp4);
            ap4_mark_display();
            break;
        case BROWSER_Cmd_PrevFile:
            if(gStudyAp4 == 0)break;
            gStudyAp4 --;
            if((gStudyAp4%DICTEXT_LINE_PAGE)!=0) {
                AP4_filename_display(gStudyAp4);
            }
            ap4_mark_display();
            break;
...
//      SendString((U8 *)"\r\nFILEBROWSER_Cmd_NextFile");
//      SDKLoadCode( TEXT_VIEWER_TEMP_CODE17 );
            if(gStudyAp4 >= (ap3_file_number - 1))break;
            gStudyAp4 ++;
            if((gStudyAp4%DICTEXT_LINE_PAGE)==0) {
                AP4_filename_display(gStudyAp4);
            }
        ...
        case BROWSER_Cmd_Enter:
            ...
            Current_Record.Ap3_num = gStudyAp4;
            State_Search =Open_File_Search;//FILEBROWSER_State_End;
            SendString((U8 *)"\r\nreadyOpenfile = ");
            send_hex_u16(gStudyAp4);
            break;
...
#pragma CODE = BROWER_PAGE_INIT_CODE

void brower_page_init(void)
{
    //display_page_num = (News_Book.Num_GoTable+1)/2;

    U32 timer;
    ap4current_mark = 0;
    g_b_first_n_p_key_flag = FALSE;
    SendString((U8 *)"\r\nUnite_Num=");
    send_hex_u16(All_unite_news.Unite_Num);
    if(All_unite_news.Unite_Num == 0) {

        if(content_back_unit) {
            content_back_unit = FALSE;
            mail_send_event( EVT_SEARCH_TO_BROWSER,BACK_TO_SUBROOT );
            b_BackSubdir = 0;

            //SDKLoadCode( FILE_TEMP_CODE01 );
            //file_close();
            //mail_send_event(EVT_KEY_PRESSED, (U16)((KEY_POWER<<8)|KBD_KEY_RELEASED) );
            return;
        }
        ...
        timer = Timer_set_timer(TIMER_S(4));
        while (!Timer_is_timeout(timer));
        */
        //mail_send_event(EVT_KEY_PRESSED, (U16)((KEY_POWER<<8)|KBD_KEY_RELEASED) );

        if(All_unite_news.Cap_Point == 0) {
            State_Search = BACK_TO_SUBROOT;
            lcd_goto_xy( 12, 32 );
            if(g_setting_list.struct_setting.language_ihm == LANG_CHINESE_SIMP) {
                lcd_puts((U8 *)"本书无屏显内容");
        ...
        //ap4current_displaypage_num = 0;
        gStudyUnite = 0;
        SDKLoadCode(BROWER_GO_PAGE_CODE);
        ready_goto_page(FALSE);
        //Cur_Jian_News.Min_Code = Cur_Unite_News.Min_PageId;
        //Cur_Jian_News.Max_Code = Cur_Unite_News.Max_PageId;
        // Cur_Run_News.Page_ID = Cur_Unite_News.Min_PageId;
        //Cur_Run_News.Index_ID = Cur_Unite_News.Min_PageId-1;
        mmi_ap3_kbd_enable();
        return;
    }
    //display_page_num = (All_unite_news.Unite_Num + 1)/2;
    //ap4current_displaypage_num = 1;
    ...
    if(content_back_unit) {
        ...
        lcd_puts( g_string );
        content_back_unit = FALSE;
        //ap4current_mark = ap4CurrentMarkBak;
        //bStudyUniteMask = false;
    } else {
        //ap4current_displaypage_num = 0;
        gStudyUnite = 0;
    }
...
#pragma CODE = DISPLAY_PAGENUM_CODE
void display_currentpage_num(void)
{
    U16 temp_data;
    SDKLoadCode( LCD_SSD1303_TEMP_CODE02 );
    lcd_clear_part_of_screen( 0, 24, LCD_WIDTH, (LCD_HEIGHT-24));
    lcd_goto_xy( 64,0 );
    Ih_set_curr_font( IDC_FONT_08 );
    temp_data = gStudyUnite + 1;
    temp_data = temp_data%100;
    time_buf[0] = temp_data/10 + '0';
    time_buf[1]  = temp_data%10 + '0';
    lcd_puts( time_buf );
}

#pragma CODE = BROWER_GO_PAGE_CODE
void ready_goto_page(Bool flag)
{
    U32 temp_addr,voice_addr;
    //temp_addr = News_Book.Point_UniteTable + 2;
    //SendString((U8 *)"\r\ncurUnite = ");
    //send_hex_u16(gStudyUnite);
    Cur_Unite_News.Cur_Unite_Num = gStudyUnite;
    ///*
    if(!flag) {
        temp_addr = News_Book.Point_UniteTable + 2;
        //temp_addr = All_unite_news.Cap_Point;
        //temp_addr += 2;
    } else {
        temp_addr = News_Book.Point_UniteTable + 6;
        //ap4current_mark -= 1;
        //temp_addr += (U32)(ap4current_displaypage_num<<2);
        temp_addr += (U32)(gStudyUnite << 2);
        //voice_addr = temp_addr + 4;
    }
    //*/
   // temp_addr = News_Book.Point_UniteTable + 2;
    //temp_addr += (U32)((gStudyUnite+1)<<2);
    Cur_Unite_News.Cap_Num = 0;
    Cur_Unite_News.Point_UniteNews = 0;
    read_data_from_nf(temp_addr, (U8 _MEM_TYPE_SLOW_ *)&voice_addr, 4);
    if(voice_addr != 0){
        read_data_from_nf(voice_addr, (U8 _MEM_TYPE_SLOW_ *)&Cur_Unite_News.Cap_Num, 2);
        Cur_Unite_News.Point_UniteNews = voice_addr + 2;
    }
/*
    ...
    send_hex_u32(Cur_Unite_News.Point_UniteNews);
//*/
    //temp_addr += (U32)(ap4current_mark<<2);
    //temp_addr += (U32)8;
    //read_data_from_nf(temp_addr, (U8 _MEM_TYPE_SLOW_ *)&Cur_Jian_News, 4);
    //SendString((U8 *)"\r\nmin_max=");
    //send_hex_u16(Cur_Jian_News.Min_Code);
    //SendString((U8 *)":");
    //send_hex_u16(Cur_Jian_News.Max_Code);
    //Current_Record.Page_num  = Cur_Unite_News.Min_PageId-1;
    Current_Record.Page_num  = 0;
    State_Search = Page_Search;//FILEBROWSER_State_End;
    //Cur_Run_News.Page_ID = Cur_Unite_News.Min_PageId;
    //Cur_Run_News.Index_ID = Cur_Jian_News.Min_Code-1;
    Cur_Cap_Num = 0;
    mmi_ap3_kbd_disable();
}
#pragma CODE = BROWER_PAGE_CODE
void Page_filename_display(U16 display_num_mark)
{
    U16 i,j,m,temp_file;
    U8 temp_str[3];
    Bool temp_flag;
...
//     lcd_goto_xy( FILEBROWSER_PosFile_StartX, FILEBROWSER_PosFile_StartY );
    ...
    if((All_unite_news.Unite_Num - display_num_mark) >= DICTEXT_LINE_PAGE ) {
        j = DICTEXT_LINE_PAGE;
    } else {
        j = DICTEXT_LINE_PAGE-1;
    }
...
//       SDKLoadCode( NAVIGATION_CODE );
//    SDKLoadCode( GET_FILE_NAME_CODE );
//      temp_flag = Get_File_Name(( U8 *)Ext, temp_file );
...
void page_mark_display(void)
{
    U16 i,temp_file;
    SDKLoadCode( LCD_SSD1303_TEMP_CODE02 );
    lcd_clear_part_of_screen( 0, 24, 10, (LCD_HEIGHT-24));
    lcd_draw_picture(5,32+20*(gStudyUnite%DICTEXT_LINE_PAGE), IDP_PLAYER_PLAY);
}
...
void brower_page(void)
{
    ...
    switch( browserfile_cmd_id ) {
        case BROWSER_Cmd_CurFile:
            //SendString((U8 *)"\r\ncurPage = ");
            //send_hex_u16(ap4current_displaypage_num);
            Page_filename_display(gStudyUnite);
            page_mark_display();
            break;
        case BROWSER_Cmd_PrevFile:
            if(gStudyUnite == 0)break;
            gStudyUnite --;
            if((gStudyUnite%DICTEXT_LINE_PAGE)!=0) {
                Page_filename_display((gStudyUnite-1));
            }
            page_mark_display();
            // SendString((U8 *)"\r\nprevPage = ");
            //send_hex_u16(ap4current_displaypage_num);
...
//      SendString((U8 *)"\r\nFILEBROWSER_Cmd_NextFile");
//      SDKLoadCode( TEXT_VIEWER_TEMP_CODE17 );
            if(gStudyUnite >= (All_unite_news.Unite_Num - 1))break;
            gStudyUnite ++;
            if((gStudyUnite%DICTEXT_LINE_PAGE)==0) {
                Page_filename_display(gStudyUnite);
            }
        ...
        case BROWSER_Cmd_Enter:
            SDKLoadCode( DISPLAY_PAGENUM_CODE );
            display_currentpage_num();
#if 1
            SDKLoadCode(BROWER_GO_PAGE_CODE);
            ready_goto_page(TRUE);
#else
            Current_Record.Page_num  = ap4current_mark;
            State_Search = Page_Search;//FILEBROWSER_State_End;
#endif
            break;
...
#pragma CODE = PRINTUNITENEWS_CODE
void printUniteNews(void)
{
    SendString((U8 *)"\r\nap4current_displaypage_num=");
    send_hex_u16(gStudyUnite);
