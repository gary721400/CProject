...
static                 Bool  b_battery_charging;
...
static void status_display_battery( void );
   ...
   switch( event )
   {
      ...
      case EVT_UPDATE_START:
         ...
         {  // The default application selected after power on
            g_startup_id_appli = mmgr_activate_mmi_appli(MMI_APPLI_STARTUP, 0);
         }
         break;
...
void status_display_battery( void )
{
   ...
   else
   {
      ...
      else 
      {
         ...
         if      (s_battery_level >= 8) IDP = IDP_STATUS_BATT_FULL;  // > ~3.8V
         else if (s_battery_level >= 6) IDP = IDP_STATUS_BATT_HIGH;  // > ~3.6V
         else if (s_battery_level >= 4) IDP = IDP_STATUS_BATT_MED;   // > ~3.4V
         else if (s_battery_level >= 1) IDP = IDP_STATUS_BATT_LOW;   // > ~3.2V
         else                           IDP = IDP_STATUS_BATT_V_LOW;
         lcd_draw_picture(84, 2, IDP);
      }
   }
}
...
char  _MEM_TYPE_SLOW_   tisiFileName[ 32 ];
char  _MEM_TYPE_SLOW_   tisiFileShortpath[ 32 ];

#pragma CODE = MMI_PLAYINTRO_CODE
...
Bool play_sbdir_mp3_file( void )
{
   ...
   if( FSGetFileSize(( char *)tisiFileShortpath, &fs_err )== 0 )
      return FALSE;
