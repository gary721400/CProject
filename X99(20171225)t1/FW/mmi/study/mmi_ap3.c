...
_MEM_TYPE_MEDSLOW_   U16               dummy_index;
...
Bool prev_press( void );
Bool next_press( void );
Bool find_caption(void);
Bool fanye_display_text(U16 key_data );
/*********************** Menus **********************/

extern   Bool                 g_b_player_on;
extern   Bool                 g_b_play_ap3;

extern _MEM_TYPE_SLOW_   Unite_News		   Cur_Unite_News;
//extern _MEM_TYPE_SLOW_   Jian_News			   Cur_Jian_News;
//extern _MEM_TYPE_SLOW_   Run_Unite_News	   Cur_Run_News;
extern _MEM_TYPE_SLOW_   All_Unite_News		All_unite_news;
extern _MEM_TYPE_SLOW_   U16                Cur_Cap_Num;
...
/*
 ...
 if( i > 32){
  ...
  if(num_current_word != 1){
	...
	u8_date[2] = j;
    SendString((U8 *)"\r\n1=");
	send_hex_u8(u8_date[0]);send_hex_u8(u8_date[1]);send_hex_u8(u8_date[2]);
}
...
Bool fanye_display_text(U16 key_data )
{
  ...
  //SendString((U8 *)"\r\nkey_id");
  //send_hex_u8(key_data);
  if(key_data == ((KEY_NEXT<<8)|KBD_KEY_LONGPRESSED)){
	  	if(b_LastPage_Flag)return TRUE;
    ...
    if(key_data == ((KEY_PREV<<8)|KBD_KEY_LONGPRESSED)){
		if(Display_Page_Num <= 1)return TRUE;
		Display_Page_Num -= 2;
		current_display_num = text_off_addr[Display_Page_Num];
		ch_display_address = ch_begin_address + (U32)current_display_num;
		mail_send_event( EVT_SEARCH_FLUSH_TEXT, 0);
...
*/
	  ...
	  if(b_ready_findword){
		...
		//player_ap3_stop();
		//init_array_v();
		if(b_HaveKeyPress)return;
		 SDKLoadCode( MMI_AP4_PLAY_STOP_CODE );
         ap4_play_stop();
//next_find:		
		if(KEY_NEXT == keyID){
	  			SDKLoadCode( MMI_AP4_KEY_PRESS_CODE04 );
	  			//changepage_key_press(keyID);
	  			if(!next_press())return;
				
			}
		else{
				SDKLoadCode( MMI_AP4_KEY_PRESS_CODE05 );
	  			//changepage_key_press(keyID);
	  			if(!prev_press())return;
			}
		//SDKLoadCode(MMI_AP4_KEY_PRESS_CODE07);
		//if(!find_caption())goto next_find;
		b_HaveKeyPress = TRUE;
...
Bool rp_record_find( U16 index )
{
   ...
   record_result = RECORD_ERROR;
   strcpy( tisiFileName, "0:\\RP\\" );
   SDKLoadCode( MMI_PLAYINTRO_CODE00 );
   if( !find_path_a(( U8 *)tisiFileName, (U8 *)tisiFileShortpath ))
   {
      err = FSDirForUnicode( "0:", ( char *)"RP", ( U8 *)atou(( U8 *)"RP" ));
   ...
   p = str_copy_u_n(( U16 *)tisiFileName, atou(( U8 *)"0:\\RP\\"), 6+1 );
...
Bool mmi_ap4_key_deal( U16 param )
{
	SendString((U8 *)"\r\nstudy_key:State_Search= ");
	send_hex_u16(param);
	SendString((U8 *)":");
	send_hex_u16(State_Search);
	if( mmi_ap3_kbd_is_disable())return TRUE;
#if (BACKLIGHT == ENABLE)
...
#  endif //#if(BACKLIGHT == ENABLE)
   ...
   if(mmi_ap4_globekey_deal(param))return  TRUE;
	if(ap4_play_state == Browser_Page_Init)return TRUE;
    //SDKLoadCode(DISPLAY_CONTEXT_CODE);
   //if(fanye_display_text(param))return TRUE;

   if((State_Search == Browser_File)||(State_Search == Browser_DirFile)||(State_Search == Browser_Page)){
	 	SDKLoadCode( BROWSER_KEY_RELEASE_CODE );
	 	//if(browser_key_release(param))return  TRUE;
	 	(void)browser_key_release(param);
	 	return  TRUE;
  	 }
   SDKLoadCode(DISPLAY_CONTEXT_CODE);
   if(fanye_display_text(param))return TRUE;
...
Bool mode_key_press( void )
{
  ...
  if(State_Search == Browser_Page){ 	
  	 ...
  	 //if(Unite_Num == 0)goto page_back; 
  	 content_back_unit = TRUE;
  	 State_Search = Browser_Page_Init;
  	}
   return FALSE;
}
...
#pragma CODE = MMI_AP4_KEY_PRESS_CODE07
Bool find_caption(void)
{
/*
	...
	temp_add = Cur_Unite_News.Point_UniteNews + (U32)((Cur_Run_News.Index_ID - Cur_Jian_News.Min_Code)<<2);
    read_data_from_nf(temp_add, (U8 _MEM_TYPE_SLOW_ *)&temp_addr, 4);
	read_data_from_nf(temp_addr, (U8 _MEM_TYPE_SLOW_ *)&temp_act.Type, 2);
	if(Type_Play_Title == temp_act.Type)return TRUE;
	if(Type_play_Caption == temp_act.Type)return TRUE;
	*/
	return FALSE;
   //(void)act_do(temp_addr, 0);
}

#pragma CODE = MMI_AP4_KEY_PRESS_CODE04
Bool next_press( void )
{
#if 1
	U32 temp_addr,temp_add;
	if(g_b_first_n_p_key_flag){
		g_b_first_n_p_key_flag = FALSE;
		Cur_Cap_Num = 0;
		return TRUE;
		}
	if(Cur_Cap_Num < (Cur_Unite_News.Cap_Num -1)){
		Cur_Cap_Num ++;
		return TRUE;
		}
	return FALSE;
/*
		Cur_Run_News.Index_ID ++;
		SendString((U8 *)"\r\ncur_min_max=");
		send_hex_u16(Cur_Run_News.Index_ID);
		SendString((U8 *)":");
		send_hex_u16(Cur_Jian_News.Min_Code);
		SendString((U8 *)":");
		send_hex_u16(Cur_Jian_News.Max_Code);
		
		if(Cur_Run_News.Index_ID > Cur_Jian_News.Max_Code){
			Cur_Run_News.Index_ID = Cur_Jian_News.Max_Code - 1;
			return FALSE;
			}
	   ...
	   SendString((U8 *)"\r\nU:p:c=");
		send_hex_u16(Cur_Unite_News.Min_PageId);SendString((U8 *)":");
		send_hex_u16(Cur_Unite_News.Max_PageId);SendString((U8 *)":");
		send_hex_u32(Cur_Unite_News.Point_UniteNews);SendString((U8 *)":");
		send_hex_u16(Cur_Jian_News.Min_Code);SendString((U8 *)":");
		send_hex_u16(Cur_Jian_News.Max_Code);SendString((U8 *)":");
		send_hex_u16(Cur_Jian_News.Point_Default);SendString((U8 *)":");
		send_hex_u16(Cur_Run_News.Index_ID);SendString((U8 *)":");
		send_hex_u16(Cur_Run_News.Page_ID);
		SendString((U8 *)"\r\nadd:addr=");
		send_hex_u32(temp_add);SendString((U8 *)":");send_hex_u32(temp_addr);
*/		
		//Current_Index = dummy_index;
		//Search_Record.news_sys.flag_index = TRUE;
#else
		{
		if(dummy_index >= News_Go.Max_Index_ThePage){
...
#pragma CODE = MMI_AP4_KEY_PRESS_CODE05
Bool prev_press( void )
{
#if 1
	  if(g_b_first_n_p_key_flag){
	...
	U32 temp_addr,temp_add;
		...
		if(Cur_Run_News.Index_ID <= Cur_Jian_News.Min_Code){
			return FALSE;
			
			if(Cur_Run_News.Page_ID <= Cur_Unite_News.Min_PageId){
				Cur_Run_News.Index_ID = Cur_Jian_News.Min_Code;
				return FALSE;
				}
			Cur_Run_News.Page_ID --;
			temp_addr = Cur_Unite_News.Point_UniteNews + (U32)((Cur_Run_News.Page_ID - Cur_Unite_News.Min_PageId)<<3);
			read_data_from_nf(temp_addr, (U8 _MEM_TYPE_SLOW_ *)&Cur_Jian_News.Min_Code, 8);
			Cur_Run_News.Index_ID = Cur_Jian_News.Max_Code;
			}
		else{
			Cur_Run_News.Index_ID --;
			}	
...
*/		
		...
		if(dummy_index <=  News_Go.page_info.Min_Index_ThePage){
			dummy_index = News_Go.Max_Index_ThePage;
			}
		else{
			dummy_index --;
			}
  ...
  switch( param )
   {
	...
	//    case ((KEY_VOL_UP<<8)|KBD_KEY_RELEASED):
	  	if((State_Search == Browser_Page)||(State_Search == Browser_File)||(State_Search == Browser_DirFile)){
