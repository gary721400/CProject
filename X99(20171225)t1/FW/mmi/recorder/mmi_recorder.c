...
ImageDeclareType( DELETE_RECFILE_CODE );
ImageDeclareType( DELETE_DISPLAY_CODE );
...
#define FILENAME_X    15
#define FILENAME_Y    26
...
char  /*_MEM_TYPE_SLOW_*/  recorder_file_name[ 10 ];
...
void remote_rec_stop(void);
void delete_rec_file(void);
void delete_file_display(void);
...
mmi_recorder_exit:
      ...
      switch( LSB( param ))
      {
		 ...
		 */	
		 ...
		 case REMOTE_DELETE_FILE:
		 remote_rec_stop();
		 SDKLoadCode(DELETE_RECFILE_CODE);
		 delete_rec_file();
		 break;
...
#pragma CODE = DELETE_DISPLAY_CODE
...
void delete_file_display(void)
{
	...
	(void)FSDelete((char *)tisiFileShortpath);
	...
	lcd_goto_xy( 19, FILENAME_Y);
    lcd_puts((U8 *)temp_name);
	delay1ms(4000);

}

#pragma CODE = DELETE_RECFILE_CODE
...
void delete_rec_file(void)
{
	...
	if( find_prev_rec_file( rec_ext )){
		SDKLoadCode(DELETE_DISPLAY_CODE);
		delete_file_display();
		}
...
Bool find_rec_file( U8 _MEM_TYPE_SLOW_ * sz_filterext )
{
   ...
   if( !nav_filelist_findname(( U8 *)cur_recorder_path, FALSE ))
   {
#if 1
      goto find_rec_file_error;
#else
      SDKLoadCode( NAVIGATION_TEMP_CODE02 );
      if( !nav_dir_make(( U8 *)cur_recorder_path ) )
      {
      ...
      //strcpy( recorderpath, "0:\\Recorder" );
   ...
   //strcpy( tisiFileName, RECORD_PATH );
      ...
      if( !nav_file_isdir())
      {
            ...
            && nav_file_name( Unicode_get_addr_string( g_string ), Unicode_get_char_space( g_string ), FS_NAME_GET, FALSE ))
         {
           ...
           // strcpy( tisiFileShortpath, RECORD_PATH);
            ...
            lcd_clear_part_of_screen( 0, 24, LCD_WIDTH, (LCD_HEIGHT - 24) );
            //lcd_goto_xy( 15, 8 );
            lcd_goto_xy( FILENAME_X, FILENAME_Y);
            Ih_set_curr_font( IDC_FONT_16 );
            //lcd_puts( g_string );
            lcd_puts((U8 *)temp_name);
            mmi_recorder_display_icon();
...
find_prev_file:
      if( !nav_file_isdir())
      {
         ...
         if( nav_file_checkext( sz_filterext ))
         {
            ...
            //strcat( tisiFileShortpath, "\\" );
            //strcpy( tisiFileShortpath, RECORD_PATH);
            strcpy( tisiFileShortpath, (char *)"0:\\");
   			strcat( tisiFileShortpath, (char *)&cur_recorder_path);
   			strcat( tisiFileShortpath, (char *)"\\");
            strcat( tisiFileShortpath, ( char *)fat_g_cur_nav->name );
...
void recorder_star_appli_deal( void )
{
 ...
 SendString((U8 *)"\r\noid init!");
  // b_oid_init_flag = FALSE;
   ...
   rec_play_state = REC_PLAY_ERROR;
   SendString((U8 *)"\r\nmode=");
   send_hex_u8(recorder_mode);
   if( recorder_mode == 1 )
   {
      //SDKLoadCode( MMI_PLAYINTRO_CODE00 );
      // Search file to play
      //search_subdir_mp3_file( "record.mp3" );
      mail_send_event( EVT_STOP_RECORDER_APPLI, 1 );
      //break;
   }
   else
   {
