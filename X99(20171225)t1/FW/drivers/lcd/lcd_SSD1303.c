...
ImageDeclareType( LCD_PICTURE_CODE );
ImageDeclareType( LCD_HEIGH8_CODE );
ImageDeclareType( LCD_COMMAND_CODE );
...
_MEM_TYPE_SLOW_ U16  lcd_display_num;
...
#pragma CODE = LCD_PICTURE_CODE
void lcd_pic(U8 x0, U8 y0, U32 IDP)
{
   //ol b_transparent_mode = FALSE;
   //ol b_animation_mode   = FALSE;
   //Bool b_suppl_page       = FALSE;
   _MEM_TYPE_SLOW_ U8  id_width;
...
//   _MEM_TYPE_SLOW_ U8  prev_transp_mask;
   _MEM_TYPE_SLOW_ U8  width_mark[32];
   _MEM_TYPE_SLOW_ U8  temp_x,temp_y,i,j,id_data,temp_width,m,n;	
...
*/	
   ...
   id_width  = Ih_get_picture_data();
   v0.height = Ih_get_picture_data();
	/*
   SendString((U8 *)"\r\nwidth:heigh=");
  send_hex_u8(id_width);
  SendString((U8 *)":");
  send_hex_u8(v0.height);
  //*/
   memset(width_mark,0,32);
   display_char_width = id_width;

/*
   if(display_char_width<=8){
   	temp_width = 1;
   	if(display_char_width%2){
		width_mark[0]= display_char_width+1;
   		}
	else{
		width_mark[0]= display_char_width;
		}
	width_mark[1]= 0;
   	}
   else{
...
///*
	...
	m = display_char_width/8;
	n = display_char_width%8;
	temp_width = m+1;
	if((n==0)){
		if(m!=0){
			temp_width --;
	...
	if(temp_width>32)temp_width=32;
	for(i=0;i<m;i++){
		width_mark[i] = 8;
		}
	if(display_char_width%2){
		width_mark[temp_width-1]= display_char_width - 8*m + 1;
		}
	else{
		width_mark[temp_width-1]= display_char_width - 8*m;
		}
	
//*/		
	for(i=0;i<v0.height;i++){
	  	for(j=0;j<temp_width;j++){
			id_data =  Ih_get_picture_data();
			display_char_width = width_mark[j];
			lcd_change_char(id_data);
			/*			
			SendString((U8 *)"\r\nid_data:0:1:2:3:temp_width:display_char_width=");
			send_hex_u8(id_data);
			SendString((U8 *)":");
			send_hex_u8(display_ready_char[0]);
			SendString((U8 *)":");
			send_hex_u8(display_ready_char[1]);
			SendString((U8 *)":");
			send_hex_u8(display_ready_char[2]);
			SendString((U8 *)":");
			send_hex_u8(display_ready_char[3]);
			SendString((U8 *)":");
			send_hex_u8(temp_width);
			SendString((U8 *)":");
			send_hex_u8(display_char_width);
			
//*/
			lcd_write_one_width(temp_x,temp_y,(display_char_width/2));
			temp_x += display_char_width/2;
	  	 	}
		temp_y ++;
		temp_x = lcd_g_lcd_x;
		/*
...
#endif
void lcd_heigh_eight(U8* string)
{
  U8 start_page;
  U8 nb_lines,temp_x,temp_y;
  U8 temp_data1,i;
  ...
  while( *string )
  {
	 ...
	 nb_lines -= ' ';
#if 1
	for( start_page = 0; start_page < 8;start_page++ )
	 {
		temp_data1 = ascii8_uncode[ nb_lines ][ start_page ];
#if 0 		
		send_hex_u8(temp_data1);
#endif		
		lcd_change_char(temp_data1);			
		lcd_write_one_width(temp_x,temp_y,(display_char_width/2));
		temp_y ++;
	 }
#if 0 	
	 SendString((U8 *)"\r\n");
#endif	 
	  temp_x = temp_x + (display_char_width/2);
	  temp_y = lcd_g_lcd_y; 
#else       
...
void lcd_puts( U8* string )
{
  ...
  // check_char_width();
   ...
   {
      SDKLoadCode(LCD_COMMAND_CODE);
      SDKLoadCode(LCD_HEIGH8_CODE);
	  lcd_heigh_eight(string);
	  return;
   }
    if( NULL == (void *)string )
      { return; }
...
#define  Wait_10ms()    timer = Timer_set_timer(TIMER_MS(30)); \
  ...
  //lcd_turn_on();
   ...
   lcd_write_command( LCD_CMD_SET_PRECHARGE );
   lcd_write_command( LCD_VAL_SET_PRECHARGE );
   ...
   lcd_write_command( LCD_CMD_PIN_CONFIG );
   lcd_write_command( LCD_VAL_PIN_CONFIG );
   ...
   /*lcd_write_command( LCD_CMD_PAGE_ADDR );
   lcd_write_command( LCD_CMD_COL_HIGH_ADDR );
   lcd_write_command( LCD_CMD_COL_LOW_ADDR | 0x02 );*/
   ...
   //lcd_write_command( LCD_ENTIRE_DISPLAY_ON );
   ...
   //Set VCOMH Level
   lcd_write_command( LCD_CMD_SET_VCOM );
   lcd_write_command( LCD_VAL_SET_VCOM );
#else

   lcd_write_command(0xAE);     //Set Display Off 
   lcd_write_command(0xB0);     //Row address Mode Setting 
   lcd_write_command(0x00); 
   lcd_write_command(0x10); 	//Set Higher Column Address of display RAM 
   lcd_write_command(0x00);     //Set Lower Column Address of display RAM 
   lcd_write_command(0xD5);    //Set Display Clock Divide Ratio/Oscillator Frequency 
   lcd_write_command(0x50);  //50  125hz 
   lcd_write_command(0xD9);  //Set Discharge/Precharge Period 
   lcd_write_command(0x22);    
   lcd_write_command(0x40);     //Set Display Start Line    
   lcd_write_command(0x81);     //The Contrast Control Mode Set 
   lcd_write_command(0x26);    
   lcd_write_command(0xA1);  //Set Segment Re-map    
   lcd_write_command(0xC8);  //Set Common Output Scan Direction    
   lcd_write_command(0xA4);  //Set Entire Display OFF/ON    
   lcd_write_command(0xA6);  //Set Normal/Reverse Display 
   lcd_write_command(0xA8);  //Set Multiplex Ration 
   lcd_write_command(0x3F); 
   lcd_write_command(0xAD);  //DC-DC Setting 
   lcd_write_command(0x80);  //DC-DC is disable 
   lcd_write_command(0xD3);  //Set Display Offset 
   lcd_write_command(0x20);  
 
   lcd_write_command(0xDB);  //Set VCOM Deselect Level 
   lcd_write_command(0x30); 
   lcd_write_command(0xDC);  //Set VSEGM Level 
   lcd_write_command(0x30); 
   lcd_write_command(0x33);  //Set Discharge VSL Level  1.8V 
...
void lcd_puts_simple( U8* str )
{
   ...
   U8   _MEM_TYPE_SLOW_    *p_string;
   _MEM_TYPE_SLOW_ U8  width_mark[2];
   _MEM_TYPE_SLOW_    U8   idx_start_char = 0;
   U8 i,j,num_ziku,temp_x,temp_y,temp_width,temp_line;
   U16 wc;
   U8 char_width;
   ...
   //if( NULL == (void *)string )
    //  { return; }
//
//  if( ih_g_curr_font == IDC_FONT_24 )goto bit_24_display;
  // SendString((U8 *)"\r\ntest 16 24 puts!");
   // Default setting
  // scroll.mask      = 0xff;
...
//*/
  // SDKLoadCode( UNICODE_TEMP_CODE03 );
   ...
   //string   = (U8 _MEM_TYPE_SLOW_*)str;   // "string" keeps the returned unicode buffer address
/*   
   SendString((U8 *)"\r\np_string=");
   for(i=0;i<40;i++){
   	send_hex_u8(g_conv_buffer[i]);
   	SendString((U8 *)":");
   	}
//*/   
	  p_string = Unicode_get_addr_string((U8 _MEM_TYPE_SLOW_*)str) + idx_start_char*2;
...
//   temp_x = lcd_g_lcd_x;
//   temp_y = lcd_g_lcd_y;
   while(*(( U16 _MEM_TYPE_SLOW_ *)p_string) != 0x0000 )
   {
      //v4.wc = *((_MEM_TYPE_SLOW_ U16 *)p_string)++;
      wc = *(( U16 _MEM_TYPE_SLOW_ *)p_string);
      p_string++; p_string++;
...
/*	  
	  ...
	  send_hex_u8(temp_y);
//*/	  
	  if((temp_x >= LCD_WIDTH)||(temp_y >= LCD_HEIGHT)){
	  	lcd_display_num --;
	  	return;
	  	}
#if 0	  
      ...
      char_width  = Ih_get_font_data();
      display_char_width = char_width;
	  
	  temp_line = display_char_width;
	  ...
	  num_ziku = char_width * (height >> 3);
/*	  
      SendString((U8 *)"\r\nv2.char_width =");
	  send_hex_u8(v2.char_width);
	  SendString((U8 *)"\r\nziku =");
//*/
	  ...
	  for(i = 0;i < height;i++){
	  	 	for(j=0;j<temp_width;j++){
				idx_start_char = Ih_get_font_data();
				display_char_width = width_mark[j];
/*				
				send_hex_u8(idx_start_char);
				SendString((U8 *)":");
//*/				
				if((temp_x + display_char_width/2)> LCD_WIDTH){
					lcd_display_num --;
					return;
					}
				lcd_change_char(idx_start_char);
/*			
			SendString((U8 *)"\r\ntemp_data1:0:1:2:3:4:5=");
			send_hex_u8(idx_start_char);
			SendString((U8 *)":");
			send_hex_u8(display_ready_char[0]);
			SendString((U8 *)":");
