...
ImageDeclareType( SCAN_OID_CODE );
ImageDeclareType( OID_INIT_CODE );

ImageDeclareType( KEYPAD_PROCESS_CODE );
ImageDeclareType( TIMER_SOFT_PROCESS_CODE );

ImageDeclareType( MMGR_KERNEL_TEMP_CODE00 );
ImageDeclareType( HP_SPEAK_CHANGE_CODE );
...
void  earphone_check       ( void );
void  earphone_speak_change( void );

void systemSysDown(void){
	if(player_ap3_is_busy())return;
...
void mmgr_task(void)
{
   ...
   // Keypad process
   //SDKLoadCode( KEYPAD_PROCESS_CODE );
   kbd_process();
   ...
   if( b_oid_init_flag )
   {
      ...
      {
         //Int_disable();
         SDKLoadCode( SCAN_OID_CODE );
         //SDKLoadCode( OID_CODE );
         scan_oid();
         if( exit_flag )
         {
   ...
   //SDKLoadCode( TIMER_SOFT_PROCESS_CODE );
   ts_process();
   ...
   SDKLoadCode( MMGR_KERNEL_TEMP_CODE00 );
   earphone_check();
...
*/
#pragma CODE = MMGR_KERNEL_TEMP_CODE00
...
void earphone_check( void )
{
   ...
   if( g_b_earphone_present != earphone_is_present())
   {
      if( !u32_earphone_time )
         u32_earphone_time = Timer_set_timer( TIMER_MS( 150 ));
      if( Timer_is_timeout( u32_earphone_time ))
      {
         u32_earphone_time = 0;
         if( g_b_earphone_present != earphone_is_present())
         {
            ...
            if( g_b_player_on )
            {
               SDKLoadCode( HP_SPEAK_CHANGE_CODE );
               earphone_speak_change();
#if 0
               if( g_b_earphone_present )
               {
                  op_disable();
...
#if( EARPHONE_DET == ENABLE )
#pragma CODE = HP_SPEAK_CHANGE_CODE
...
void mmgr_task_init( void )
{
   ...
   if( FS_mount( &SD_Cache_Drive )== FS_DRIVE_ERROR )
   {
      //RESET_SYS();
   }
   //SD1_fast_mode();
