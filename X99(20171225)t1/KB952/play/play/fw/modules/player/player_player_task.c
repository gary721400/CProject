...
void player_task(void)
{
   ...
   if( g_b_player_on )
   {
      ...
      if( audio_is_wav == 0 )
      {
		 ...
		 ///*
		 current_play_time = AudioMP3GetPlayTime( PLAY_TIME_MODE );
		 if(count_AB == 3){
		  	 if(( point_B - current_play_time)<=500){
			 	gendu_tiaozhen();
		  	 	}
	  	}
		 //*/
...
Bool some_index_dowith( void )
{
   U16 dic_temp,mp3_temp,ap4_temp,v_adc,v_dec,v_min,v_max,pause_temp,prev_temp,next_temp;
   U16 diy_min,diy_max;
	if(g_setting_list.struct_setting.bluetooth){
		...
		pause_temp = OID2_MP3_PAUSE;
		prev_temp = OID2_MP3_PREV;
		next_temp = OID2_MP3_NEXT;
		...
		pause_temp = DPR3_MP3_PAUSE;
		prev_temp = DPR3_MP3_PREV;
		next_temp = DPR3_MP3_NEXT;
	  ...
	  /*
	   if(Current_Index == mp3_temp){
		...
		if(Current_Index == pause_temp){
			//mail_send_event(EVT_KEY_PRESSED, (U16)((KEY_VOL_UP<<8)|KBD_KEY_RELEASED));
			mail_send_event( EVT_REMOTE, REMOTE_MP3_PLAY_PAUSE);
			 return TRUE;
			}

		if(Current_Index == prev_temp){
			 mail_send_event( EVT_REMOTE, REMOTE_MP3_PREV);
			 return TRUE;
			}

		if(Current_Index == next_temp){
			 mail_send_event( EVT_REMOTE, REMOTE_MP3_NEXT);
			 return TRUE;
			}
