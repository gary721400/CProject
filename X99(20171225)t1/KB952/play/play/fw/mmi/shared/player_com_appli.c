...
void call_mmi_appli(U8 idx, U8 evt_id, U16 evt_param)
{
  ...
  //  SendString((U8 *)"\r\nid_module:id:param=");
  //  send_hex_u8(idx);SendString((U8 *)":"); send_hex_u8(evt_id);SendString((U8 *)":"); send_hex_u16(evt_param);
   switch( idx )
   {
      case 0                     :  status_mmi_appli     (evt_id, evt_param); break;
      //case MMI_APPLI_IDLE        :  swap_code( MMI_IDLE_CODE_FLAG );    idle_mmi_appli       (evt_id, evt_param); break;
      //case MMI_APPLI_MSTORAGE    :  swap_code( USB_STORAGE_CODE_FLAG ); storage_mmi_appli    (evt_id, evt_param); break;
      case MMI_APPLI_PLAYER      :  player_mmi_appli     (evt_id, evt_param); break;
