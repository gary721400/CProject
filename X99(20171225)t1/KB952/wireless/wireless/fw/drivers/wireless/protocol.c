...
// /*
   SendString((U8 *)"protocol init \r\n");
 {
   ...
   RadioGetFuses();
	...
	MSB0(machine_id) = temp[ 0 ];
	MSB1(machine_id) = temp[ 1 ];
	MSB2(machine_id) = temp[ 2 ];
	MSB3(machine_id) = temp[ 3 ];
...
/*
    if( !power_is_vbus_present())
    {
	 ...
	 if( g_kbd_key == (( U16 )KEY_MODE<< 8 |( U16 )KBD_KEY_RELEASED))
      {
        green_led_off();
        red_led_off();
