...
void reinit_oid_pen( void )
{
   ...
   MrmInit();                // 初始化mpr7601和sensor GC0307
   temp = MrmGetChipID();    // 正确值: 0x7601
   SendString((U8 *)"temp=");
   send_hex_u16(temp);
   SendString((U8 *)" ");
   if( temp != 0x7601 )
      return;
   temp = MrmI2cRead( 0x00 );  // 正确值: 0x99
   send_hex_u8(temp);
   SendString((U8 *)" ");
   if( temp != 0x99 )
      return;
   
   temp = MrmI2cRead( 0x01 );  // 正确值: 0x6A
   send_hex_u8(temp);
   SendString((U8 *)" ");
   temp = MrmI2cRead( 0x02 );  // 正确值: 0x70
   send_hex_u8(temp);
   SendString((U8 *)" ");
   temp = MrmI2cRead( 0x03 );  // 正确值: 0x02
   send_hex_u8(temp);
   SendString((U8 *)" ");
   temp = MrmI2cRead( 0x04 );  // 正确值: 0x80
   send_hex_u8(temp);
   SendString((U8 *)"\r\n");
    
   /* sensor校正，进入前应用白纸将摄像头覆盖 */
#if 0
   FSGetFileSize(( char *)paraPath, &fserr );
   if( fserr )
   {
      if( ret = AdjustExpose())     // 设置sensor曝光
         ret = WinInit();           // 设置sensor窗口位置
      if( ret )
      {
