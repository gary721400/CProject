...
Bool record_index_dowith( void )
{
   U16 dic_temp,mp3_temp,ap4_temp,v_adc,v_dec,v_min,v_max,record_temp,play_temp,stop_temp;
	if(g_setting_list.struct_setting.bluetooth){
		record_temp = OID2_RECORD_B_INDEX;
		stop_temp = OID2_RECORD_O_INDEX;
		play_temp = OID2_RECORD_P_INDEX;
		}
	else{
		record_temp = DPR3_RECORD_B_INDEX;
		stop_temp = DPR3_RECORD_O_INDEX;
		play_temp = DPR3_RECORD_P_INDEX;
		}
	 if( Current_Index == record_temp){
            mail_send_event( EVT_REMOTE, REMOTE_REC_START);
            return TRUE;
         }
	 
	  if( Current_Index == stop_temp){
            mail_send_event( EVT_REMOTE, REMOTE_REC_STOP);
            return TRUE;
         }
	  
	   if( Current_Index == play_temp){
...
///*
	  if(record_index_dowith())return;
	  if(g_b_recorder_on)return;
...
Bool some_index_dowith(void)
{
     if( Current_Index == RECORD_B_INDEX){
            mail_send_event( EVT_REMOTE, REMOTE_REC_START);
            return TRUE;
         }
	 
	  if( Current_Index == RECORD_O_INDEX){
            mail_send_event( EVT_REMOTE, REMOTE_REC_STOP);
            return TRUE;
         }
	  
	   if( Current_Index == RECORD_P_INDEX){
            mail_send_event( EVT_REMOTE, REMOTE_REC_PLAY);
            return TRUE;
         } 

	   if((Current_Index >= RECORD_V_1_INDEX)&&(Current_Index <= RECORD_V_7_INDEX)){
            mail_send_event( EVT_REMOTE,(Current_Index - RECORD_V_1_INDEX + REMOTE_VOL0) );
            return TRUE;
         } 

      if( Current_Index == RECORD_GO_AP4_INDEX ){
            mail_send_event( EVT_REMOTE, REMOTE_AP4_MODE);
            return TRUE;
         } 	   
	   return FALSE;
}
...
Bool some_index_dowith( void )
{
   ...
   U16 delete_temp;
	if(g_setting_list.struct_setting.bluetooth){
		...
		delete_temp = OID2_RECORD_D_INDEX;
		//record_temp = OID2_RECORD_B_INDEX;
		//stop_temp = OID2_RECORD_O_INDEX;
		//play_temp = OID2_RECORD_P_INDEX;
	...
	/*
	 if( Current_Index == record_temp){
	  ...
	  if((Current_Index >= diy_min)&&(Current_Index <= diy_max)){
			...
			mail_send_event( EVT_REMOTE, REMOTE_REREAD_MODE);
			return TRUE;
			}
#endif		   
	  if(Current_Index == delete_temp){
	  	if(g_setting_list.struct_setting.bluetooth){
			mail_send_event( EVT_REMOTE, REMOTE_DELETE_FILE);
			return TRUE;
	  		}
	  	}
	  if(Current_Index == dic_temp){
		...
		mail_send_event( EVT_REMOTE, REMOTE_DIC_MODE);
		return TRUE;
	  	}
	   if(Current_Index == mp3_temp){
		...
		mail_send_event( EVT_REMOTE, REMOTE_MP3_MODE);
		return TRUE;
	  	}
