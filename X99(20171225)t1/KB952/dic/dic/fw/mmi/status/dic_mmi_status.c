...
ImageDeclareType( CHANGE_PROJECT_CODE );
...
void status_mmi_appli(U8 event, U16 param)
{
   switch( event )
   {
      ...
      case EVT_BACK_TO_TOP:
         ...
         SDKLoadCode(CHANGE_PROJECT_CODE);
         change_project();
         break;
      ...
      case EVT_UPDATE_START:
         ...
         {  // The default application selected after power on
            ( void )mmgr_activate_mmi_appli(MMI_APPLI_DIC, 0);
...
//***********************************************
      ...
      case EVT_MAILBOX_EVT_FULL:
         ...
         lcd_goto_xy(16, 24); lcd_puts(( U8 *)"System error");
         while(1);
...
Bool search_subdir_mp3_file( char *str )
{
   ...
   link_filename(str);
   return find_path_u(( U16 *)tisiFileName, ( U8 *)tisiFileShortpath );
...
#pragma CODE = CHANGE_PROJECT_CODE
